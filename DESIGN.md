...
- `words` 字段现在恒为 `null`（见 1.2：`yrc` 拿不到），但**从第一天就存在于结构里**，
  这样将来实现加密调用后不必迁移缓存格式。
- **`lyric.offsetMs` 自决策 41 起语义固定为「per-track 原始值」**：全局偏移开启时前端本地忽略
  这个字段、改用共享的全局值，daemon 侧不感知这一开关、照旧原样写入 per-track 偏移。
