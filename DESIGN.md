...
│   │   ├── LyricSource.{h,cpp} 读快照 + 持有时间轴 + 逐帧推进
│   │   └── tests/
│   └── plasmoid/
...
| 10 | 播放器选择：自动跟随"最近变为 Playing 的"，用 `kde:pid` 压制浏览器自带服务，外加可编辑黑名单；匹配不上时降级为"只信 pbi" |
...
| # | 决策 |
|---|---|
| 25/32/36 | 可读性是**两个独立维度**：底板 `plate ∈ {none, ksvg, solid}`（**默认 `ksvg`**）× 描边 `stroke: bool`（**默认 `false`**——默认底板已足够压住壁纸，描边要 8 份 Text 副本，不该默认开）。毛玻璃**删除**（1.3：结构上不可能），第三档换成纯半透明色块 |
...
| 30 | 字体跟随 Plasma 通用字体设置，**颜色必须与主题脱钩**（歌词压在壁纸上，`Kirigami.Theme.textColor` 在浅色主题下会彻底翻车），且均可自定义 |
| 14 | 四种无词状态：① 未播放 → 可自定义文案，默认「当前未在播放」，可置空；② 查不到 → 独立文案，默认空；③ 间奏 → 空；④ 暂停 → 保留最后一行不动 |

### 质量与发布
| # | 决策 |
|---|---|
| 33 | ⚠️ **未定案**。已定：`plasma-lyricsd --explain "<标题>" "<歌手>"` 离线打印匹配全过程；**不做设置里的日志面板**。未定：日志载体——访谈中选了「独立日志文件」，但 user service 的 stdout 本来就免费进 journal，需确认是"只写文件（要自行实现轮转）"还是"journal + 可配置文件（默认关闭）"。见第 8 节末行 |
| 34 | 测试必须项见第 6 节 |
...
规则：`^[^\s：:]{1,12}[：:]\s*.+$`，自歌词开头连续扫描，**遇到第一个不命中的行即停止**。

**已验证命中**（`老街北` 1299289240 的真实前 7 行，实测通过，第 8 行 `若能再相见` 正确终止扫描）：
`编曲/伴奏混音：闹闹丶`、`调教：FFF君`、`混音：小欧Ω`、`曲绘：偶尤大肥羊`、
`PV/封面设计：Ansa`、`文案：铭言君，Ansa`、`歌姬：洛天依`

**已知漏判**（本规则不会过滤，会显示在桌面上）：
- `Mix&Mastering: Foo Bar` —— 冒号前 13 字符，超出 `{1,12}`
- `Vocal 调整：某某 某某` —— 冒号前含空格，被 `[^\s：:]` 排除

这两种写法在同人音乐里可能出现，但**本机曲库里尚未抓到实例**。收到实例后再放宽（放宽头部会
同时提高误判真歌词的概率），届时把该实例加进 fixture。

**已知误判风险**：形如 `我说：你听` 的真歌词会命中本规则。风险由"仅扫描前奏区 + 遇到第一个
不命中即停止"两个条件约束——只有当这类句子恰好出现在**第一句真歌词之前**时才会被吞掉。
接受此风险；替代方案（关键词白名单）的失效模式更糟：它会在歌曲中段任意位置吞掉歌词，且无声。
...
| 独立日志文件的轮转 | ⚠️ **待确认**：Q33 你答 "(b)" 指向独立日志文件。本文档暂按 "journal 为主 + 可配置日志文件（默认关闭）" 写。若你要的是"只写文件、不进 journal"，需要自行实现轮转，请告知后修订本节与第 3 节 #33 |
