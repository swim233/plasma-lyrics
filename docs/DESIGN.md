...
| 54 | **多歌词源与 AMLL**：后端全局顺序默认 `local → netease → amll`，单曲首选按现有指纹持久化并移到链首，其余已配置源仍按全局顺序回退且每次只查询一次。缓存分为用户首选、实际命中、按 provider 失败冷却三种状态；旧 `fingerprint` 只迁移为实际结果，旧全局 `miss` 不抑制 AMLL。正常播放可先发布已有回退歌词；只有保留项是非空、状态为 `ok` 的有效歌词时，强制重搜失败才不清屏，否则必须发布 `not-found` 或具体错误终态。首选源冷却结束后仅在明确的播放轮次事件后台重试且一轮最多一次。MPRIS `Seeked` 到达时立即用目标位置和单调时钟重新锚定，通常作为手工跳转抑制轮次；考虑少数播放器也会在自然循环时发送该信号，只有同一套自然过尾证据独立成立时仍保留一轮事件。对 pbi 等不发 `Seeked` 的来源，曲尾→开头只有在 `(单调时间差 × Rate)` 已足以消耗上一采样点的曲尾剩余时长、且当前位置接近预期的循环后相位时才算新轮次；轮询、`Seeked` 与脏 `PropertiesChanged(Position)` 共用判据并在采样后统一重锚，普通回退仍只重新对轴且同一次回跳不会重复。协议极限是：若用户跳转发生在恰好与自然过尾相同的时刻且目标又与预期相位吻合，MPRIS 没有提供 seek 原因，客户端没有可观察信息区分两者；错误的曲长、Rate 或严重延迟也可能让非标准播放器越出容差。手动换源/恢复自动/重搜经带预期指纹的会话 D-Bus 控制接口执行，均为强制请求，由世代号阻止旧回调提交。手动换源与恢复自动先查链首源（所选源；恢复自动时为全局顺序中首个已配置源）：本地已有该曲的映射与非空歌词时直接显示该缓存、不联网，非空手工覆盖优先于缓存，与普通解析一致；没有才联网搜索。重搜不查这份缓存，始终联网。联网的强制请求一律绕过实际结果映射、回退源缓存与负缓存，用户主动操作不等失败冷却。AMLL 缓存 JSONL 索引，24 小时后条件刷新，校验成功才原子替换；缓存版本纳入规范化索引与内容 URL，索引元数据记录来源，来源变化时强制无条件重验证并丢弃旧 ETag/Last-Modified。历史记录以关联为边做传递合并，稳定引用优先使用最早组；标题、艺人和已知 ISRC 冲突阻止误合并，具体 `rawLyricFile` 仅作为内容版本。AMLL 匹配保留 Live/Remix/Cover/重制等后缀：版本一致或双方无标记为正常层，仅单边标记为低层，明确冲突排除。TTML 在 `core/` 解析为现有整首模型，接受带单位或无单位秒时间、保留 `span` 间有效空白，并把官方 `<translations>/<translation>/<text for>` 按行 id 映射；翻译语言按 BCP-47 主语言、脚本和地区确定性选择。保存行/词时间及来源元数据；`x-bg` 与罗马音不拼入主歌词，前端首版仍按行显示。音乐播放平台（MPRIS 来源）与歌词 provider 是两个独立概念 |
| 55 | **部件是 SQLite 零接触的快照消费者**：`lyric.offsetMs` 是生效偏移，快照另带 `globalOffsetEnabled`；偏移调整经带预期指纹的 `AdjustOffset(deltaMs)` / `ResetOffset` 写入守护进程并立即重发快照。单曲模式必须有当前歌曲和歌词引用；全局模式在有当前歌曲时仍校验预期指纹，在确实没有当前歌曲且预期指纹也为空时允许从部件菜单调整。2 秒健康检查只做 daemon pid 存活检测。唯一例外是配置对话框中的 `GlobalConfig`：它为同步报告保存失败而直连 `LyricStore`，成功后以 `RefreshGlobalOffset` 通知 daemon 调用只写快照的窄重发布函数；该函数校验当前 MPRIS 指纹仍与已解析状态一致，绝不借偏移刷新触发完整解析。通知失败不改变已经成功落盘的设置，下次 daemon 启动会读取它。因此 `LyricStore` 只有守护进程与配置对话框两类调用方。**决策 75 起，`Control` 接口另加四个只管记日志、不写任何状态的方法**：`NoteConfigChange`/`NoteSaveFailed`/`NoteRestartRequested`/`NoteRestartFinished` |
...
| 30 | **字族默认跟随 Plasma 通用字体设置（`Kirigami.Theme.defaultFont.family`），可按部件覆盖**（2026-09-23 修订，原为只跟随、不可配）；**字号、字重、颜色是部件自己的配置，不跟随主题**——颜色尤其必须脱钩（歌词压在壁纸上，`Kirigami.Theme.textColor` 在浅色主题下会彻底翻车）。颜色两种编辑方式并列：Plasma 取色器（`org.kde.kquickcontrols.ColorButton`）+ 十六进制输入框，共用同一个 `#RRGGBB` / `#AARRGGBB` 字符串——QML 颜色的 `toString()` 保留 alpha 字节，所以取色器不会把半透明默认值（`#99000000`、`#cc000000`）变成不透明。**字族**：`<form>FontFamily` 管歌词主行与第二行，空串 = 跟随 Plasma；曲目信息另有 `<form>TrackInfoFontSameAsLyrics`（默认 true，跟随歌词的**实际生效**字族）+ `<form>TrackInfoFontFamily`（空串 = 跟随 Plasma）——两个键而非一个带保留值的字符串，与 `secondLineColorEnabled`／`secondLineColor` 同构。候选由 `FontCatalog`（`frontend/qmlmodule`）给出：只列可缩放、且覆盖拉丁或中日韩文字的字族（本机 712 → 207，滤掉 `.pcf` 位图字体、乱码名与只覆盖其他文字的字体），字重写进名字的字族（如 `Noto Sans CJK HK Light`）原样保留。Qt 按当前语言环境只列一种名字（plasmashell 在 zh_CN 下列「微软雅黑」，C 下列「Microsoft YaHei」）且无公开别名 API，所以**直接链接 fontconfig** 读出各语言名：设置页的模糊搜索（类 fzf 子序列匹配）对全部名字生效，存储名到列出名的映射也依据它——**不用 `QFontDatabase::hasFamily()`**，实测它在 zh_CN 下对英文名的结果取决于进程里是否已调用过 `families()`。存储的字族未安装时**显式**回退 Plasma 字体（交给 fontconfig 替代会落到 `Noto Sans` 一类，不一定是 Plasma 字体），配置值原样保留、字体装回即恢复，设置页以「<原名>（未安装）」一项显示。**字重**（2026-09-23 修订，取代原「固定 6 档」）：列表按实际生效字族的非斜体实有字面生成（`QFontDatabase::styles()` + `weight()`，同字重去重），100 的整数倍显示译名，其余显示字体自带样式名（如 Noto Sans CJK SC 的 DemiLight = 316），只有一个字面时禁用。存储仍是 Int，不迁移；显示与渲染都按 CSS Fonts 4 的匹配规则取最近的实有字面，所以请求的字重总是真实字面，fontconfig 不再合成粗体。原记录「Qt 会吸附到最近的实有档位」不准确：缺档时 fontconfig 会合成粗体（Noto Sans CJK SC 选 600 时实际是 Medium 加合成粗体，按新规则改为 Bold）。只有设置页里某次编辑改变了某段的实际生效字族时，才把该段字重写回为最近档（换歌词字体且曲目信息「与歌词相同」时两段一起写回）；字体更新、卸载或迁入的配置只在显示与渲染时取最近档，不写回 |
...
| # | 决策 |
|---|---|
| 33 | 诊断四件套：**① journal**——daemon 是 systemd user service，消息处理器无条件安装并写 stderr，由 systemd 收进 journal（`journalctl --user -u plasma-lyricsd -f`），输出形态按 stderr 的实际去向三分，见决策 64。「直通 journal、零实现成本」的原始设想不成立：Qt6 若链接 `libsystemd`（Arch 的 qt6-base 即如此），其默认消息处理器在非 tty 时走 `sd_journal_send()` 而非 stderr，且 journald 只按 syslog 优先级着色、不认应用自己打的级别文字，因此原生着色与去重都需要显式实现；**② 可配置日志文件**——`logging/fileEnabled` 默认 `false`，路径 `logging/filePath` 默认 `~/.local/share/plasma-lyrics/plasma-lyricsd.log`，开启后与 journal 并行写；**③ `plasma-lyricsd --explain "<标题>" "<歌手>"`**——离线打印匹配全过程，调匹配逻辑时的主力工具；**④ 分类日志与调试开关**——七个 `QLoggingCategory`（`plasmalyrics.daemon`/`.resolver`/`.mpris`/`.provider.netease`/`.provider.amll`/`.provider.local`/`.provider.qq`），默认 info；`logging/debug` 打开对应 debug 级输出；三层过滤规则叠加、优先级依次升高——kdebugsettings 写入的 `qtlogging.ini` < 该开关调用的 `QLoggingCategory::setFilterRules` < `QT_LOGGING_RULES` 环境变量，因此开关开启时在 kdebugsettings 里单独关闭某个分类不生效（仅指 debug 开关；kdebugsettings 设的 info/warning 阈值仍生效）。`daemon/plasma-lyricsd.categories` 随 `BUILD_PLASMOID=ON` 安装，这七个分类因此在 kdebugsettings 中可见；决策 75 把部件自己的 `plasmalyrics.config` 也加进了同一个文件，所以 kdebugsettings 中实际可见的是八个分类，但 `logging/debug` 只管这七个，不管 `plasmalyrics.config`。**不做设置里的日志面板**（实现要两天、半年用两次，`--explain` 覆盖同一需求且离线可重复）|
| 75 | **设置变更日志**：四类改动全部记录——①「歌词服务」页保存 daemon INI（`BackendConfig::save`，`~/.config/plasma-lyrics/plasma-lyricsd.ini`）；②「全局设置」页保存全局偏移进 `LyricStore` 的 SQLite `setting` 表（`GlobalConfig::save`，键 `globalOffsetEnabled`/`globalOffsetMs`）；③桌面外观/面板外观/文本三页的每个 per-instance kcfg 键，前端经 `Plasmoid.configuration` 的 `valueChanged` 信号观察，因此程序化写入（如应用启动时的文本设置迁移）与用户手改**不加区分地**一并记录——挂钩只有这一处，用户明确要求两者都记；④「保存并重启服务」的重启请求与结果各记一行。**机制**：保存时先读一次当前存储值，与将写入的值比较，写入成功后按变化的键各记一行，归到新 `QLoggingCategory` `plasmalyrics.config`（info 级，默认严重级别 info，不产生 debug 输出，且不受 `logging/debug` 开关影响——该开关只管 daemon）。前端跑在 plasmashell 进程里，这些行落进 plasmashell 自己的 journal：`journalctl --user QT_CATEGORY=plasmalyrics.config`；不加 `-u` 会与 daemon 的行混在一起。**转发**：同时经既有 D-Bus `io.github.swim233.PlasmaLyrics.Control` 接口新增的四个只管记日志、不改变任何状态的方法转发给 daemon——`NoteConfigChange(store, applet, form, key, oldValue, newValue)`/`NoteSaveFailed(store, reason)`/`NoteRestartRequested()`/`NoteRestartFinished(success, error)`；daemon 收到后在 `plasmalyrics.daemon` 分类记同一行，因此也会出现在 `journalctl --user -u plasma-lyricsd` 与可选日志文件里。两侧渲染这行文本的代码只有一份——共享 formatter `core/log/configlog.h`，双方都 include，保证两份记录逐字节相同。**转发是尽力而为**（`QDBus::NoBlock`，不等回复、不处理错误，用户决定）：daemon 未运行、或是不带这四个方法的旧版本时，只有前端那份存在；「保存并重启服务」的请求通知与重启发出的 SIGTERM 存在竞态，通知几乎总是先到但不保证。**daemon 信任调用方**：不做鉴权或校验，伪造行是否值得为此设计一套 D-Bus 认证机制，用户已判定不值得（决策日期 2026-09-21）。**行格式**（沿用决策 61 的 `key=value` 风格）：`store` 取 `ini`/`db`/`applet`；`applet` 行额外带 `applet=<Plasmoid.id> form=desktop\|panel`；`key` 是存储键原名（如 `providers/enabled`、`globalOffsetMs`、`desktopFontSize`）；`old`/`new` **一律经 `quoted()` 加引号**，数字与布尔值也不例外（`true`/`false`，列表用逗号拼接），使两个字段不论键的类型是什么都以同一种方式解析。级别上保存失败与重启失败为 warning，变更、重启请求与重启成功为 info。示例：`config changed store=ini key=providers/enabled old="local,netease" new="local,netease,qq"`；`config changed store=db key=globalOffsetMs old="0" new="-300"`；`config changed store=applet applet=12 form=desktop key=desktopFontSize old="34" new="36"`；`config save failed store=ini reason="proxy-url-invalid"`；`config restart requested`；`config restart finished result=ok`。保存失败的 `reason` 取 `proxy-url-invalid`/`ini-access-error`/`ini-format-error`/`db-open-failed`/`db-write-failed` 之一，且失败时不产生任何变更行（没有真正落盘任何东西）。`network/proxyUrl` 可能带凭据：**是否变化的判定用存储原文（含凭据）比较**，只有写进日志的 `old`/`new` 两个值经过脱敏——能解析成 `scheme://host:port` 时按此渲染（`ProxySpec::display()`），解析不出时走 `ProxySpec::redactedForLog()`，两者都不安全时渲染成 `redacted(length=N)`。因此**只改凭据、host/port 不变**时仍会记一行，只是 `old`/`new` 渲染出来完全相同（如 `old="socks5://h.test:1080" new="socks5://h.test:1080"`）——这是有意的：这行只证明「值变了」，不泄露变成了什么。其余键一律原样记录，判定与渲染用同一个值，不存在这种脱敏后撞车的情形。**不覆盖**手改 INI 文件或 appletsrc 文件（没有任何观察点可挂）；daemon 重启后的 `started …` 行仍会显示重启后的生效配置。新分类 `plasmalyrics.config` 加进 `daemon/plasma-lyricsd.categories`，随 kdebugsettings 可见|
| 34 | 测试必须项见第 6 节 |
